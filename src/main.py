...
if __name__ == "__main__":
    ...
    dataset.write_kb_data(kb_train_path, article_range = (100, 400))
    dataset.write_kb_data(kb_test_path, article_range = (20, 100))
    #model1.cross_valid(kb_train_path)
    model1.fit(kb_train_path)
    model1.predict(kb_test_path, kb_pred_path)
    print "model1 valid score:", model1.score(kb_test_path, kb_pred_path)[0]
        
    dataset.extract_crf_features()
    dataset.write_crfpp_data(crf_train_path, article_range = (0, 20))
    dataset.write_crfpp_data(crf_test_path, article_range = (400, dataset.article_num))
    model2.fit(crf_train_path)
    model2.predict(crf_test_path, crf_pred_path)
    print "model2 without adding data",model2.score(crf_test_path, crf_pred_path)[0]
    
    os.system("cat %s %s > %s" % (crf_train_path, kb_pred_path, crf_extend_train_path))
    model2.fit(crf_extend_train_path)
    model2.predict(crf_test_path, crf_pred_path)
    print "model2 after adding data",model2.score(crf_test_path, crf_pred_path)[0]

    dataset.write_crfpp_data(crf_train_path, article_range = (0, 100))
    model2.fit(kb_pred_path)
    model2.predict(crf_test_path, crf_pred_path)
    print "model2 Upper Bound",model2.score(crf_test_path, crf_pred_path)[0]
